...
if($admin_code == ADMIN_CODE){
	...
	if(!$dao->is_user_exist($username)){
		...
		if($username && $password){
			...
			$inserted_id = $dao->insert_user($object);
			$result = false;
			...
			if($inserted_id){
				...
				if($result){
					...
					$text_message = "Регистрация прошла успешно, на ваш email отправлено письмо для активации учетной записи" ;
